...
function error(e) {
    return new Error(isNaN(e) === !1 ? STATUS[e] : "ENAMETOOLONG" === e.code ? STATUS[414] : STATUS[404]);
}
...
function wrapper(my) {
    function output(res, head, after, path, stat) {
        ...
        res.send(head), STORY.body = head), my.cache === !0 && (STORY.path = path, STORY.mtime = stat.mtime.getTime());
    }
    ...
    return my.json === !0 && (build = json), my.sync === !0 ? function(req, res, next) {
        ...
        var path = decodeURIComponent(parse(req).pathname).replace(/\/{2,}/, "/"), prova = normalize(join(my.root, path));
        if (~prova.indexOf("\x00")) return next(error(400));
        if (prova <= my.root) return next(error(403));
        var stat = fs.statSync(prova);
        if (void 0 !== stat) {
            if (void 0 !== STORY.mtime && STORY.mtime === stat.mtime.getTime() && STORY.path === prova) return res.send(STORY.body);
            if (stat.isDirectory() === !1) return my.statico(req, res, next);
            var head;
            my.json === !0 ? head = Object.create(null) : (head = header, head = head.replace(/{{path}}/g, path), 
            "/" != path && (head += '<a href="../">../</a>\n'));
        ...
        if (my.strictMethod === !0 && "GET" !== req.method && "HEAD" !== req.method) return next(error(404));
        var path = decodeURIComponent(parse(req).pathname).replace(/\/{2,}/, "/"), prova = normalize(join(my.root, path));
        return ~prova.indexOf("\x00") ? next(error(400)) : prova <= my.root ? next(error(403)) : void fs.stat(prova, function(err, stat) {
            if (null !== err) return next(error(err));
            ...
            "/" != path && (head += '<a href="../">../</a>\n')), fs.readdir(prova, function(err, files) {
                ...
                for (var after = "", cc = files.length - 1, i = 0, ii = files.length; ii > i; i++) !function(file) {
                    return my.exclude !== !1 && my.exclude.test(file) ? 0 === cc ? output(res, head, after, prova, stat) : void cc-- : my.dotfiles === !0 && "." === file[0] ? 0 === cc ? output(res, head, after, prova, stat) : void cc-- : void fs.stat(prova + PATH.sep + file, function(err, stats) {
                        if (null !== err) return next(error(err));
                        var r = build(head, after, file, stats);
...
function index(root, opt) {
    ...
    var options = opt || Object.create(null), my = {
        ...
        date: options.date === !1 ? !1 : !0,
        size: options.size === !1 ? !1 : !0,
        priority: options.priority === !1 ? !1 : !0,
        cache: options.cache === !1 ? !1 : !0,
        strictMethod: Boolean(options.strictMethod),
        sync: Boolean(options.sync),
        json: Boolean(options.json),
        statico: options["static"] === !1 ? function(req, res, next) {
            return next();
...
try {
    var STATUS = require("http").STATUS_CODES, PATH = require("path"), fs = require("fs"), normalize = PATH.normalize, resolve = PATH.resolve, join = PATH.join, parse = require("parseurl"), serve = require("serve-static");
